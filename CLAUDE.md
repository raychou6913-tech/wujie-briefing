...
| 系統 | 主場 |
|---|---|
| **ERP**（本 repo）| **業務調度** — 業務員 / 採購 / 設計 / 財務日常工作流 |
| **Forge** (`wujie-studio`) | **3D 立體** — 主題發想 → 效果圖 → 四視圖 → 3D 模型 → 公仔開發 |
| **IP Bible** (`ip-bible.pages.dev`) | **2D 平面** — 角色創作 → CI → 圖庫 → 平面周邊 |

三系統**共用 R2 圖庫**（`ASSETS_BUCKET` + 飛書 `ip_assets` 主表）+ 統一命名規則。詳見 memory `asset_naming_convention.md`。ERP 從共用圖庫拉素材給業務員提案 / 送審 / 物流追蹤。
